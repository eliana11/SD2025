...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::fprintf(stderr, "Uso: %s <tarea_json_string>\n", argv[0]);
        return 1;
    }

    std::string task_json_string = argv[1];

    // Ahora usa la función encapsulada para parsear el JSON
    MiningTaskParams params = parseMiningTaskJSON(task_json_string);

    if (!params.parse_success) {
        std::cerr << "Error: No se pudieron parsear los parámetros de la tarea desde el JSON." << std::endl;
        return 1; // Salir con error si el parsing falló
    }

    // DEBUG: Imprime los parámetros parseados
    std::cout << "DEBUG: PrevHash: " << params.prev_hash_str << std::endl;
    std::cout << "DEBUG: Dificultad: " << params.difficulty_prefix_str << std::endl;
    std::cout << "DEBUG: Start Nonce: " << params.start_nonce << std::endl;
    ...
    int target_prefix_len = hexStringToBytes(params.difficulty_prefix_str.c_str(), target_prefix_bytes);
    if (target_prefix_len == -1) {
        std::cerr << "Error: Prefijo de dificultad hexadecimal inválido: " << params.difficulty_prefix_str << std::endl;
        return 1;
    }
    ...
    unsigned char h_final_hash[16] = {0};
    char h_final_number_string[MAX_NUMBER_STR_LEN] = {0};

    // --- Lógica de minado CPU/GPU ---
#ifdef __CUDACC__
    ...
    // Después de la búsqueda, preparas el resultado:
    if (found_solution) {
        char final_hash_hex_str[33];
        for (int i = 0; i < 16; ++i) {
            std::sprintf(&final_hash_hex_str[i * 2], "%02x", h_final_hash[i]);
        }
        final_hash_hex_str[32] = '\0';

        std::string nonce_str_for_json(h_final_number_string);
        std::cout << "DEBUG: Nonce encontrado (std::string): [" << nonce_str_for_json << "]" << std::endl;

        guardarResultadoJSON(nonce_str_for_json, final_hash_hex_str, elapsed_time_ms);
    } else {
        std::cout << "DEBUG: No se encontró solución en el rango dado." << std::endl;
        guardarResultadoJSON("", "", elapsed_time_ms);
    }
