...
#include <stdio.h>
#include <string.h> // Para strlen en el host
#include <iostream>
#include <stdint.h> // Para uint32_t, etc.
#include <assert.h> // Para assert, si se usa en la librería MD5
#include <limits.h> // Para CHAR_BIT (en algunas implementaciones)
...
int main(int argc, char* argv[]) {
    ...
    const unsigned long long TOTAL_SEARCH_SPACE = (unsigned long long)NUM_BLOCKS * THREADS_PER_BLOCK * MAX_ATTEMPTS_PER_THREAD;
    ...
    CHECK_CUDA_ERROR(cudaMemset((void*)d_found_flag, 0, sizeof(int))); // CORRECCIÓN AQUÍ
    ...
    int h_found_flag = 0;
    CHECK_CUDA_ERROR(cudaMemcpy(&h_found_flag, (const void*)d_found_flag, sizeof(int), cudaMemcpyDeviceToHost)); // CORRECCIÓN AQUÍ
    ...
    // 6. Liberar memoria
    cudaFree((void*)d_found_flag); // CORRECCIÓN AQUÍ
